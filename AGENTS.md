...
- This runs on a **Linux** VM; CodexBar is a macOS app. Only the cross-platform SwiftPM targets build/run here: `CodexBarCore`, `CodexBarCLI`, `AdaptiveRefreshCore`, `AdaptiveReplayKit`, `AdaptiveReplayCLI`, plus the Linux test bundles (`TestsLinux`, `Tests/AdaptiveReplay*`). The macOS GUI app and its bundle (`CodexBar`, `CodexBarWidget`, `CodexBarClaudeWatchdog`, `CodexBarClaudeWebProbe`, `Tests/CodexBarTests`) are `#if os(macOS)`-gated in `Package.swift` and cannot build/run on Linux — do not try `Scripts/compile_and_run.sh`, `package_app.sh`, or `make start` here.
- Toolchain: Swift 6.3.3 (via swiftly, matching CI's `build-linux-cli`), already on `PATH` for login shells via `~/.profile` and for others via `~/.bashrc`. If `swift` is missing in a fresh shell, `source ~/.local/share/swiftly/env.sh`.
- Dev loop mirrors CI's Linux job: `swift build --product CodexBarCLI` and `swift test --parallel`. Prefer `swift test --parallel` over `make test`/`Scripts/test.sh` (that harness is tuned for the macOS sharded suite).
- Run the CLI (the runnable "app" on Linux), e.g. `"$(swift build --product CodexBarCLI --show-bin-path)/CodexBarCLI" config validate --format json`. `usage` against real accounts / `--source web` need macOS + real credentials/Keychain; do not attempt live provider probes here.
- SwiftLint gotcha: `swiftlint` (via `Scripts/install_lint_tools.sh swiftlint` → `./.build/lint-tools/bin/swiftlint --strict`, or `Scripts/lint.sh lint-linux`) fatally fails with `Loading libsourcekitdInProc.so failed` unless the toolchain lib dir is on `LD_LIBRARY_PATH`. Export `LD_LIBRARY_PATH="$HOME/.local/share/swiftly/toolchains/6.3.3/usr/lib:$LD_LIBRARY_PATH"` before running it. `Scripts/lint.sh lint-linux` also runs node-based portable checks (node is available).
